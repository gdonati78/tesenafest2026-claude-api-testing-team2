...
| `POST tasks` with `content: ""` → 400 `INVALID_ARGUMENT_VALUE` (20, `error_extra.argument: content`, `expected: minlen`); `"   "` → 400 `ITEM_CONTENT_EMPTY` (61)    | Different tags for empty and blank content. The spec has no 400 body schema: `toMatchObject` |
| `POST tasks` without `content` (or `null`) → 400 `ARGUMENT_MISSING` (19, `error_extra.argument: content`)                                                            | Nothing is created                                                                           |
| Unparseable `due_string` or `due_date` → 400 `BAD_REQUEST` (42, `"Invalid date format"`), nothing created                                                            | Not a silent drop: the whole create is rejected                                              |
| 400 bodies carry a per-request `error_extra.event_id` and a `retry_after` that grows with each error                                                                 | Never assert `error_extra` with `toEqual`; pick the stable fields                            |
